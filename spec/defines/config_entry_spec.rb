...
describe 'ssh::config_entry' do
  ...
  context 'with no paramater is provided' do
    ...
    it 'fails' do
      expect {
        is_expected.to contain_define(subject) # rubocop:disable RSpec/NamedSubject
  ...
  context 'with mandatory params set' do
    ...
    it do
      is_expected.to contain_concat('/test/path').with('ensure' => 'present',
                                                       'owner'          => 'test_owner',
                                                       'group'          => 'test_group',
                                                       'mode'           => '0644',
                                                       'ensure_newline' => true)
    ...
    it do
      is_expected.to contain_concat__fragment('/test/path Host test_host').with('target' => '/test/path',
